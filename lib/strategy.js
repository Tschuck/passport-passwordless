    ...
    this.passwordless._tokenStore.authenticate(token, uid.toString(), function (error, valid, referrer) {
        ...
        if (error) {
            ...
            that._verify(req, uid, function (err, user, info) {
                ...
                var parsedReferrer;
                try {
                    parsedReferrer = JSON.parse(referrer)
                } catch (ex) {
                    that.error('no referrer provided!');
                }
                //if no token reuse is allowed, invalidate the token after the first authentication
                if (that.options.allowTokenReuse != true || (parsedReferrer && parsedReferrer.allowTokenReuse != true || parsedReferrer.allowTokenReuse>=1)) {
